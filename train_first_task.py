...
def train(local_rank, args):
    ...
    model = LLM2VecED(args.class_num//5 + 1, args.input_map) # define model
    model.to(device)
        ...
        for ep in range(args.epochs):
            ...
            for bt, batch in enumerate(tqdm(stage_loader)):
                ...
                if stage > 0 and args.distill != "none":
                    ...
                    prev_valid_mask_op = torch.nonzero(torch.BoolTensor([item in prev_learned_types for item in range(args.class_num//5 + 1)]).to(device))
            ...
            if ((ep + 1) % args.eval_freq == 0 and args.early_stop) or (ep + 1) == args.epochs: # TODO TODO
                ...
                with torch.no_grad():
                    ...
                    for batch in tqdm(eval_loader):
                        ...
                        valid_mask_eval_op = torch.BoolTensor([idx in learned_types for idx in range(args.class_num//5 + 1)]).to(device)
